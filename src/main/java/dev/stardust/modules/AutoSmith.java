    ...
    private boolean isValidEquipmentForTrimming(ItemStack stack) {
        if (stack.getItem() instanceof ArmorItem armor) {
            ...
            switch (armor.getType()) {
                case BOOTS -> correctMaterial = bootsType.get().equals(armorMaterial);
                case HELMET -> correctMaterial = helmetType.get().equals(armorMaterial);
                case LEGGINGS -> correctMaterial = leggingsType.get().equals(armorMaterial);
                case CHESTPLATE -> correctMaterial = chestplateType.get().equals(armorMaterial);
            }

            if (!correctMaterial) return false;
            NbtCompound metadata = stack.getNbt();
            if (metadata.contains("trim", NbtElement.COMPOUND_TYPE)) {
                if (!overwriteTrims.get()) return false;
                NbtCompound trim = metadata.getCompound("trim");
        ...
        if (timer >= tickRate.get()) {
            ...
            switch (modeSetting.get()) {
                case Trim -> {
                    ...
                    if (!output.isEmpty()) {
                        NbtCompound metadata = output.getNbt();
                        if (metadata.contains("trim", NbtElement.COMPOUND_TYPE)) {

                            NbtCompound trimData = metadata.getCompound("trim");
                            String pattern = trimData.getString("pattern");
                            String material = trimData.getString("material");
