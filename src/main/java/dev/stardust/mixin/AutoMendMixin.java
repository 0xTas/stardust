...
public abstract class AutoMendMixin extends Module {
    ...
    private void replaceElytra() {
        for (int n = 0; n < mc.player.getInventory().main.size(); n++) {
            ...
            if (stack.getItem() == Items.ELYTRA) {
                if (Utils.hasEnchantments(stack, Enchantments.MENDING) && stack.getDamage() != 0) {
    ...
    private void hijackOnTick(CallbackInfo ci) {
        ...
        if (chest.isEmpty() || chest.getItem() != Items.ELYTRA || !Utils.hasEnchantments(chest, Enchantments.MENDING) || chest.getDamage() == 0) {
            replaceElytra();
        }
